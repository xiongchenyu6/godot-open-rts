...
var _origin_position = null
var _patrol_position = null
var _moving_to_patrol_position = true
var _current_target_position = null
...
func _init(patrol_position):
	_patrol_position = patrol_position


func _ready():
	_origin_position = _unit.global_position_yless
	_setup_scan_timer()
	_start_moving(_patrol_position)
...
func _process(_delta):
	if _sub_action_mode == "moving" and _has_reached_current_destination():
		_switch_destination()


func is_moving_to_patrol_position():
	return _moving_to_patrol_position
...
func _start_moving(target_position):
	_current_target_position = target_position
	_replace_sub_action(Moving.new(target_position), "moving")
...
func _has_reached_current_destination():
	return (
		_unit.global_position_yless.distance_to(_current_target_position * Vector3(1, 0, 1))
		<= DESTINATION_REACHED_DISTANCE
	)


func _switch_destination():
	_moving_to_patrol_position = not _moving_to_patrol_position
	var next_position = _patrol_position if _moving_to_patrol_position else _origin_position
	_start_moving(next_position)
...
func _on_sub_action_finished():
	...
	if finished_mode == "moving":
		...
		_start_moving(_current_target_position)
